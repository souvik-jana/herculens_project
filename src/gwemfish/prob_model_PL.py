...
class ProbModel(hcl.NumpyroModel):
    ...
    def model(self):
        ...
        source_amp = numpyro.sample('source_amp', dist.TruncatedNormal(4.0, 2.0, low=2.4, high=10.0))#jnp.asarray(4.0) #numpyro.sample('source_amp', dist.TruncatedNormal(4.0, 2.0, low=2.4, high=10.0))
        source_R_sersic = numpyro.sample('source_R_sersic', dist.TruncatedNormal(0.05, 0.04, low=0.001))#jnp.asarray(0.05) #numpyro.sample('source_R_sersic', dist.TruncatedNormal(0.05, 0.04, low=0.001))
        source_n = numpyro.sample('source_n', dist.Uniform(0.005, 2.5))#jnp.asarray(1.0) #numpyro.sample('source_n', dist.Uniform(0.005, 2.5))
        source_e1 = numpyro.sample('source_e1', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))#jnp.asarray(0.0) #numpyro.sample('source_e1', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))
        source_e2 = numpyro.sample('source_e2', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))#jnp.asarray(0.0) #numpyro.sample('source_e2', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))
        # True source center position (hardcoded)
        source_center_x = numpyro.sample('source_center_x', dist.TruncatedNormal(0.2, 0.1,low = 0.2-0.1,high = 0.2+0.1)) #jnp.asarray(0.2)# error = 0.098 arcsec
        source_center_y = numpyro.sample('source_center_y', dist.TruncatedNormal(0.0, 0.2,low = -0.005,high = 0.005)) #jnp.asarray(0.0) #numpyro.sample('source_center_y', dist.TruncatedNormal(0.0, 0.00002,low = -0.00001,high = 0.00001)) #jnp.asarray(0.1)
        ...
        cx_l = jnp.asarray(0.0) #numpyro.sample('light_center_x', dist.Normal(0., self.pix_scl/2))
        cy_l = jnp.asarray(0.0) #numpyro.sample('light_center_y', dist.Normal(0., self.pix_scl/2))
        e1_l = jnp.asarray(0.0) #numpyro.sample('light_e1', dist.TruncatedNormal(0., 0.2, low=-0.3, high=0.3))
        e2_l = jnp.asarray(0.0) #numpyro.sample('light_e2', dist.TruncatedNormal(0., 0.2, low=-0.3, high=0.3))
        light_amp = jnp.asarray(0.0) #numpyro.sample('light_amp', dist.TruncatedNormal(8, 2.0, low=0.0, high=9.5))
        light_R_sersic = jnp.asarray(1.0) #numpyro.sample('light_R_sersic', dist.TruncatedNormal(1.0, 0.5, low=0.88, high=1.15))
        light_n = jnp.asarray(3.0) #numpyro.sample('light_n', dist.Uniform(2.4, 5.))
        ...
        lens_theta_E = numpyro.sample('lens_theta_E', dist.Uniform(0.5, 5))#jnp.asarray(2.0) #error = 0.03
        lens_e1 = numpyro.sample('lens_e1', dist.Uniform(-0.65, 0.65))#jnp.asarray(0.0) #numpyro.sample('lens_e1', dist.Uniform(-0.065, 0.065))
        lens_e2 = numpyro.sample('lens_e2', dist.Uniform(-0.65, 0.65))#jnp.asarray(0.0) #numpyro.sample('lens_e2', dist.Uniform(-0.065, 0.065))
        lens_gamma = numpyro.sample('lens_gamma', dist.Uniform(1, 5))#jnp.asarray(2.0) #error = 1.1
        # True lens center position (hardcoded)
        lens_center_x = jnp.asarray(0.0) #jnp.asarray(0.0)
        lens_center_y = jnp.asarray(0.0) #jnp.asarray(0.0)
        ...
        # Noise parameter
        sigma_bkg = jnp.asarray(1.0e-4) #numpyro.sample('noise_sigma_bkg', dist.Uniform(low=0.0002, high=0.012))
...
class ProbModel_GW(hcl.NumpyroModel):
    """Probabilistic model for joint EM+GW parameter estimation."""
    ...
    def model(self):
        ...
        # Lens mass parameters
        lens_theta_E = numpyro.sample('lens_theta_E', dist.Uniform(0.5, 5))#jnp.asarray(2.0) #error = 0.03
        lens_e1 = jnp.asarray(0.0) #numpyro.sample('lens_e1', dist.Uniform(-0.65, 0.65))#jnp.asarray(0.0) #numpyro.sample('lens_e1', dist.Uniform(-0.065, 0.065))
        lens_e2 = jnp.asarray(0.0) #numpyro.sample('lens_e2', dist.Uniform(-0.65, 0.65))#jnp.asarray(0.0) #numpyro.sample('lens_e2', dist.Uniform(-0.065, 0.065))
        lens_gamma = numpyro.sample('lens_gamma', dist.Uniform(1, 5))#jnp.asarray(2.0) #error = 1.1
        # True lens center position (hardcoded)
        lens_center_x = jnp.asarray(0.0) #jnp.asarray(0.0)
        lens_center_y = jnp.asarray(0.0) #jnp.asarray(0.0)
...
class ProbModel_EM_GW(hcl.NumpyroModel):
    """Probabilistic model for joint EM+GW parameter estimation."""
    ...
    def model(self):
        ...
        # Source light parameters
        source_amp =numpyro.sample('source_amp', dist.TruncatedNormal(4.0, 2.0, low=2.4, high=10.0))
        source_R_sersic = numpyro.sample('source_R_sersic', dist.TruncatedNormal(0.05, 0.04, low=0.001))
        source_n = numpyro.sample('source_n', dist.Uniform(0.005, 2.5))
        source_e1 = numpyro.sample('source_e1', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))
        source_e2 = numpyro.sample('source_e2', dist.TruncatedNormal(0.0, 0.06, low=-0.3, high=0.3))
        # True source center position (hardcoded)
        source_center_x = numpyro.sample('source_center_x', dist.TruncatedNormal(0.2, 0.1,low = 0.2-0.09,high = 0.2+0.09)) #jnp.asarray(0.2)
        source_center_y = jnp.asarray(0.0) #numpyro.sample('source_center_y', dist.TruncatedNormal(0.0, 0.00002,low = -0.00001,high = 0.00001)) #jnp.asarray(0.1)
        ...
        # Lens mass parameters
        lens_theta_E = numpyro.sample('lens_theta_E', dist.Uniform(2-0.03, 2+0.03))
        lens_e1 = jnp.asarray(0.0) #numpyro.sample('lens_e1', dist.Uniform(-0.065, 0.065))
        lens_e2 = jnp.asarray(0.0) #numpyro.sample('lens_e2', dist.Uniform(-0.065, 0.065))
        lens_gamma = numpyro.sample('lens_gamma', dist.Uniform(2-0.5, 2+0.5))#jnp.asarray(2.0) #
        # True lens center position (hardcoded)
        lens_center_x = jnp.asarray(0.0) #jnp.asarray(0.0)
        lens_center_y = jnp.asarray(0.0) #jnp.asarray(0.0)
        gamma1 = jnp.asarray(0.0) #numpyro.sample('lens_gamma1', dist.Uniform(-0.006, 0.005))
        ...
        # Noise parameter
        sigma_bkg = jnp.asarray(5.0e-3) #numpyro.sample('noise_sigma_bkg', dist.Uniform(low=0.0002, high=0.012))
