...
class ProbModel_GW_only(hcl.NumpyroModel):
    ...
    def model(self):
        ...
        # GW parameters
        T_star = numpyro.sample('T_star', dist.Uniform(1e4, 1e8))  # in seconds
        dL = numpyro.sample('dL', dist.Uniform(0.0, 61800.0))  # in Mpc
        ...
        lens_gamma = numpyro.sample('lens_gamma', dist.Uniform(1.1, 3.0))
        # True lens center position (hardcoded)
        lens_center_x = jnp.asarray(0.0)
        lens_center_y = jnp.asarray(0.0)
        ...
        for i in range(self.n_images):
            ...
            minx = -20 #mean_x - delx[i]/2
            maxx = 20 #mean_x + delx[i]/2
            miny = -20 #mean_y - dely[i]/2
            maxy = 20 #mean_y + dely[i]/2
