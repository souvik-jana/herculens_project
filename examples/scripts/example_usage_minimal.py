...
# Setup
kwargs_lens, x_image_true, y_image_true, lens_mass_model = setup_lens( lens_model_list=DEFAULT_LENS_MODEL_LIST, kwargs_lens=DEFAULT_KWARGS_LENS, zl=DEFAULT_ZL, zs=DEFAULT_ZS, source_pos=DEFAULT_SOURCE_POS_GW )
...
#em_obs, lens_image = simulate_em( kwargs_lens=kwargs_lens, kwargs_source=DEFAULT_KWARGS_SOURCE, kwargs_lens_light=DEFAULT_KWARGS_LENS_LIGHT, lens_mass_model=lens_mass_model, source_model_class=source_model, lens_light_model_class=lens_light_model, pixel_grid=pixel_grid, psf=psf, noise_class=noise_simu, kwargs_numerics=DEFAULT_KWARGS_NUMERICS, exposure_time=exposure_time, seed=87651)
x_img_gw, y_img_gw, gw_obs, data_GW, lens_gw = simulate_gw( source_pos=DEFAULT_SOURCE_POS_GW, kwargs_lens=kwargs_lens, lens_mass_model=lens_mass_model, cosmology=cosmology, zl=DEFAULT_ZL, zs=DEFAULT_ZS, lens_model_list=DEFAULT_LENS_MODEL_LIST)

# Model (likelihood)
probmodel = ProbModel(
    n_images=4,
    gw_observations=gw_obs,
    em_observations=None, # No EM data
    lens_image=None,
    lens_gw=lens_gw,
    noise=None
)
...
keys_to_exclude = ['D_dt', 'lens_e1']  # Add any other keys to exclude
keys_to_include = [k for k in input_params.keys() if k not in keys_to_exclude]
...
# Plot the results:
import corner
def plot_corner(samples_dict, truths, title):
    samples_array = jnp.column_stack([samples_dict[k] for k in keys_to_include])
    figure = corner.corner(
        samples_array,
        labels=keys_to_include,
        truths=[truths[k] for k in keys_to_include],
        show_titles=True,
        title_fmt=".3f",
    ...
    plt.show()
# Plot Fisher samples from covariance matrix
plot_corner(fisher_samples, truths, "Fisher Samples from Covariance Matrix")
# Plot MCMC samples from Fisher approximation model
plot_corner(samples_approx, truths, "MCMC Samples from Fisher Approximation Model")
print("Corner plots generated!")
