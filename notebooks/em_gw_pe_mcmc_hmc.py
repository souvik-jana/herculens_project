...
plt.tight_layout()
plt.savefig("lens_system_epl_shear.pdf", bbox_inches='tight')
plt.close()
...
ax.set_ylabel('count')
plt.tight_layout()
plt.savefig("noise_map_and_histogram.pdf", bbox_inches='tight')
plt.close()
...
for text in legend.get_texts():
    text.set_color('white')
fig.tight_layout()
plt.savefig("simulated_lensing_images_with_gw_positions.pdf", bbox_inches='tight')
#plt.show()
plt.close()
...
fig.tight_layout()
plt.savefig("simulated_lensing_images_with_gw_positions_pixel_grid.pdf", bbox_inches='tight')
#plt.show()
plt.close()
...
# If the samples pickle file does not exist, run MCMC to generate them
save_dir = '../data'
save_file_name = 'inference_results_PE_EM.pkl'
if not os.path.exists(os.path.join(save_dir, save_file_name)):
    ...
    # Save the samples, summary_dict, extra_fields, and mcmc objects using pickle
    import pickle
    import os
    save_dir = '../data'
    os.makedirs(save_dir, exist_ok=True)
    inference_dict_to_save = { 'samples': samples, 'summary_dict': summary_dict, 'extra_fields': extra_fields, 'mcmc': mcmc }
    with open(os.path.join(save_dir, save_file_name), 'wb') as f:
        pickle.dump(inference_dict_to_save, f)
else:
    print(f"Samples file '{save_file_name}' already exists. Loading samples from file.")
    with open(os.path.join(save_dir, save_file_name), 'rb') as f:
        inference_dict_loaded = pickle.load(f)
    samples = inference_dict_loaded['samples']
    summary_dict = inference_dict_loaded['summary_dict']
    extra_fields = inference_dict_loaded['extra_fields']
    mcmc = inference_dict_loaded['mcmc']
...
#         jnp.logical_not(outside_validity),  #         jnp.logical_and(outside_validity, jnp.logical_not(quadratic_too_large))  # Outside but OK
#     )
